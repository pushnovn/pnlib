...
namespace PN.Text
{
    public class Search
    {
        ...
        public static List<Tuple<string, double>> Fuzzy(
            string searchString,
            List<string> sourceStrings,
            bool isRusEngKeyboardLayoutIndependent = false,
            bool isTransliterateSearch = false)
        {
            return Fuzzy(searchString, sourceStrings, isRusEngKeyboardLayoutIndependent, isTransliterateSearch, out long evaluateTime);
        }
        
        public static List<Tuple<string, double>> Fuzzy(
            string searchString, 
            List<string> sourceStrings,
            bool isRusEngKeyboardLayoutIndependent,
            bool isTransliterateSearch,
            out long evaluateTime)
        {
            evaluateTime = 0;
            ...
            {
                Words = searchString.Split(' ').Select(word => new Word()
                {
                    Text = word.ToLower(),
                    Codes = GetKeyCodes(codeKeys, word)
                }).ToList(),
            };
            var keyboardLayoutSearchObj = isRusEngKeyboardLayoutIndependent ? null : new AnalizeObject()
            {
                Words = searchString.Split(' ').Select(word =>
                {
                    string keyboardLayoutStr = Utils.Converters.SwitchKeyboard(word.ToLower());
                    return new Word()
                    {
                        Text = keyboardLayoutStr,
                        Codes = GetKeyCodes(codeKeys, keyboardLayoutStr)
                    };
                }).ToList(),
            };
            var transliterateSearchObj = isTransliterateSearch ? null : new AnalizeObject()
            {
                Words = searchString.Split(' ').Select(w =>
                {
                    string transliterateStr = Transliterate(w.ToLower(), Translit_Ru_En);
                    return new Word()
                    {
                        Text = transliterateStr,
                        Codes = GetKeyCodes(codeKeys, transliterateStr)
                    };
                }).ToList(),
            };
            ...
            {
                var sourceAnalizeObject = new AnalizeObject()
                {
                    Words = sourceString.Split(' ').Select(word => new Word()
                    {
                        Text = word.ToLower(),
                        Codes = GetKeyCodes(codeKeys, word)
                    }).ToList(),
                ...
                var originalCost = GetRangePhrase(sourceAnalizeObject, originalSearchObj, false);
                var keyboardLayoutCost = keyboardLayoutSearchObj == null ? double.MaxValue : GetRangePhrase(sourceAnalizeObject, keyboardLayoutSearchObj, true);
                var transliterateCost = transliterateSearchObj == null ? double.MaxValue : GetRangePhrase(sourceAnalizeObject, transliterateSearchObj, true);

                result.Add(new Tuple<string, double>(sourceString, new double[]{ originalCost, keyboardLayoutCost, transliterateCost }.Min()));
            }
        ...
        //            languageType = 3;
        //        }
        //        // Проверка транслитерационной строки
        ...
        //            languageType = 3;
        //        }
        //        result.Add(new Tuple<string, string, double, int>(sampl.Rus.Original, sampl.Eng.Original, cost, languageType));
        //    }
        //    return result;
        //}


        private static double GetRangePhrase(AnalizeObject source, AnalizeObject search, bool translation)
        {
            if (!source.Words.Any())
            {
                if (!search.Words.Any())
